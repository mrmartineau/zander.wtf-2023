...
module.exports = function (eleventyConfig) {
  ...
  // Likes
  eleventyConfig.addCollection('likes', async () => {
    const likesPath = urlJoin('https://otter.zander.wtf/api/tag/zm:link', {
      query: {
        limit: 50,
      },
    })
    const likesData = await axios.get(likesPath, {
      headers: {
        Authorization: `Bearer ${process.env.OTTER_API_KEY}`,
      },
    })
    return likesData.data.data
  })
  // Films
